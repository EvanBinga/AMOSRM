...
class AMO {
		...
		if(isset($res['response']['auth'])&&$res['response']['auth']==1){
			$this->logged=true;}
				...
				if(isset($this->acc))
				{
					...
					$this->CONTACT_EMAIL_ID=array_values($this->acc['custom_fields']['contacts']);$this->CONTACT_EMAIL_ID=$this->CONTACT_EMAIL_ID[2]['id'];
					$this->CONTACT_PHONE_ID=array_values($this->acc['custom_fields']['contacts']);$this->CONTACT_PHONE_ID=$this->CONTACT_PHONE_ID[1]['id'];
				}
	...
	public function get_contacts($param)
	{
		...
		if($this->last_code!=200)return array();
...
public    function get_field($contact,$field_id)
	...
	public function add_contacts($contacts)
	{
		...
		$link='https://'.$this->subdomain.'.amocrm.ru/api/v2/contacts';
		$res=$this->sendPOST($link,$contacts);
		if($this->last_code!=200)return -1;
		return $res['_embedded']['items'];
	}
    ...
    public function add_note($note)
	{
		...
		$param['add']=array($note);
		$res=$this->sendPOST($link,$param);
		if($this->last_code!=200)return 0;
		return $res['_embedded']['items'];
	...
	public function add_contact_info($name,$phone='',$email='',$linked_lead='')
	{
        ...
        if(!empty($email) || !empty($phone))
        {
            ...
            $c=$this->find_emails($email);

            if(empty($c))$c=$this->find_phones($phone);
            if(!empty($c))
            {
            	...
            	$res=$cont['id'];
            	$lead=array('contacts_id'=>$cont['id']);
            	$this->edit_lead($lead,$linked_lead);    
            }else $res=$this->add_contact($cont);
